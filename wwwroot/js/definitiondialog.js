...
export async function showDefinitionDialog(callback, values) {
  ...
  disableElements([
    ...
    'definitionType', 
    'definitionBehaviour', 
    'definitionReadonlyYes', 'definitionReadonlyNo'
  ], isEditing);

  document.getElementById('definitionDialogTitle').textContent = isEditing ? 'Edit property' : 'Add property';
  document.getElementById('definitionDialogOk').textContent = isEditing ? 'Save changes' : 'Add property to collection';
  document.getElementById('definitionName').value = values?.name || '';
  document.getElementById('definitionType').value = values?.type || 'STRING';
  document.getElementById('definitionDescription').value = values?.description || '';
  document.getElementById('definitionBehaviour').value = values?.propertyBehavior || 'DYNAMIC';
